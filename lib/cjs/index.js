...
const htmlPlugin = (configuration = { files: [] }) => {
    ...
    function collectEntrypoints(htmlFileConfiguration, metafile) {
        const entryPoints = Object.entries((metafile === null || metafile === void 0 ? void 0 : metafile.outputs) || {})
            .filter(([, value]) => {
            if (!value.entryPoint) {
                return false;
            }
    ...
    function findNameRelatedOutputFiles(entrypoint, metafile, entryNames) {
        ...
        if (entryNames) {
            ...
            return Object.entries((metafile === null || metafile === void 0 ? void 0 : metafile.outputs) || {})
                ...
                if (entryNames) {
                    ...
                    const findFilesWithSameVariablesRegexString = escapeRegExp(entryNames
                        .replace("[name]", name !== null && name !== void 0 ? name : "")
                        .replace("[dir]", dir !== null && dir !== void 0 ? dir : "")).replace("\\[hash\\]", REGEXES.HASH_REGEX);
            ...
            return Object.entries((metafile === null || metafile === void 0 ? void 0 : metafile.outputs) || {})
                .filter(([key]) => {
                return (path_1.default.parse(key).name === pathOfMatchedOutput.name &&
                    path_1.default.parse(key).dir === pathOfMatchedOutput.dir);
            })
                .map((outputData) => {
                // Flatten the output, instead of returning an array, let's return an object that contains the path of the output file as path
                return { path: outputData[0], ...outputData[1] };
    ...
    async function injectFiles(dom, assets, outDir, publicPath, htmlFileConfiguration) {
        ...
        for (const outputFile of assets) {
            ...
            if (htmlFileConfiguration.hash) {
                const hashableContents = htmlFileConfiguration.hash === true
                    ? `${Date.now()}`
                    : htmlFileConfiguration.hash;
                targetPath = `${targetPath}?${crypto_1.default
                    .createHash("md5")
                        ...
                        if (htmlFileConfiguration.findRelatedCssFiles) {
                            if (entrypoint === null || entrypoint === void 0 ? void 0 : entrypoint.cssBundle) {
                                relatedOutputFiles.set(entrypoint.cssBundle, {
                                    path: entrypoint === null || entrypoint === void 0 ? void 0 : entrypoint.cssBundle,
                                });
                            }
                        }
    ...
    async function cleanSourceFileAsync(filepath) {
        await fs_1.default.promises.rm(path_1.default.resolve(filepath));
    }
};
